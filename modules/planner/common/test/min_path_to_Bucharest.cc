...
// given. The target node is Bucharest, and the user can specify the initial
// city from which the search algorithm will start looking for the minimum path
// to Bucharest.
//
...
//
// Please note that this exercise is academic in nature and that the distances
// between the cities may not be correct compared to the latitude and longnitude
// differences in real life. Thanks to parthi2929 for noticing this issue. In a
// real application you would use some kind of exact x,y position of each city
// in order to get accurate heuristics. In fact, that is part of the point of
...
void PathSearchNode::PrintNodeInfo() { cout << " " << CityNames[city] << "\n"; }

int main(int argc, char *argv[]) {
  // creating map of Romania
  for (int i = 0; i < MAX_CITIES; i++)
    for (int j = 0; j < MAX_CITIES; j++) RomaniaMap[i][j] = -1.0;
